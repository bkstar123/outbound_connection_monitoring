 ...
 END {
     ...
     if (max_connection_count>ENVIRON["threshold"]) {
          if (getline < "dump_taken.lock" < 0) {
              ...
              system("/tools/dotnet-dump collect -p $pid -o $dump_file > /dev/null && echo \"Memmory dump has been collected. Uploading it to Azure Blob Container insights-logs-appserviceconsolelogs\" && /tools/azcopy copy \"$dump_file\" \"$sas_url\" > /dev/null && echo \"Memory dump has been uploaded to Azure Blob Container insights-logs-appserviceconsolelogs\"")
          }
     }
 }' | sort -k2,2nr
echo "--------------------------------------------------------------------------------"
